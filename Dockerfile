...
RUN sudo apt-get update -y && sudo apt-get install -y pciutils libfftw3-dev libfftw3-doc r-base-dev libcurl4-openssl-dev libxml2-dev libssl-dev libmagick++-6.q16-dev
...
RUN Rscript -e "options(repos = c(CRAN = 'https://cran.r-project.org')); install.packages(c('R.cache', 'knitr', 'rmarkdown', 'DiagrammeR', 'reticulate', 'keras')); keras::install_keras()"
